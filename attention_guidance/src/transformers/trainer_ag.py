...
class TrainerAG:
    ...
    def _setup_wandb(self):
        ...
        if self.is_world_master():
            ...
            wandb.init(name=os.getenv("WANDB_NAME"), project=os.getenv("WANDB_PROJECT", "huggingface"), config=vars(self.args))
    ...
    def train(self, model_path: Optional[str] = None):
        ...
        for epoch in train_iterator:
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                log_loss = self._training_step(model, inputs, optimizer)
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0 or (
                    ...
                    if (self.args.logging_steps > 0 and self.global_step % self.args.logging_steps == 0) or (
                        ...
                        logs: Dict[str, float] = {}
                        if self.args.ag:
                            logs["total_loss"] = log_loss[0]
                            logs["mlm_loss"] = log_loss[1]
                            logs["ag_loss"] = log_loss[2]
                        else:
                            logs["mlm_loss"] = log_loss
    ...
    def _log(self, logs: Dict[str, float], iterator: Optional[tqdm] = None) -> None:
        ...
        if is_wandb_available() and os.getenv("WANDB_DISABLED") != "true":
            if self.is_world_master():
    ...
    def _training_step(
        ...
        # Use both the attention initialization loss and masked language modeling loss
        if not self.args.ag:
            loss = mlm_loss
        else:
            ag_loss = self.compute_ag_loss(inputs, outputs[-1], self.args.device, self.args.attn_head_types)
            loss = mlm_loss + ag_loss * self.args.scale * self.linear_schedule_for_scale()        
        ...
        if self.args.ag:
            return (loss.item(), mlm_loss.mean().item(), ag_loss.mean().item())
        else:
            return loss.item()
